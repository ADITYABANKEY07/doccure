  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-white shadow-sm font-sans">
      ...
      <div className="nav hidden md:flex h-20 items-center justify-around">
        ...
        <div className="nav flex gap-8 font-medium text-gray-700">
          <Link className="hover:text-blue-500 transition" to="/home">
            Home
          </Link>
          <Link className="hover:text-blue-500 transition" to="/contact">
            Contact
          </Link>
          <Link className="hover:text-blue-500 transition" to="/booking">
            Booking
          </Link>
        </div>
        ...
        <div className="flex items-center gap-4">
          {!isLoggedIn ? (
            <>
              <Link
                className="flex items-center gap-2 bg-gradient-to-r from-blue-500 to-cyan-500 text-white px-6 py-2 rounded-full shadow-md hover:opacity-90 transition"
                to="/login"
              >
                <LockKeyhole size={18} /> Login
              </Link>
              <Link
                className="flex items-center gap-2 bg-slate-800 text-white px-6 py-2 rounded-full shadow-md hover:bg-slate-900 transition"
                to="/signup"
              >
            ...
            <div className="mt-auto pb-6">
              {isLoggedIn ? (
                <div className="bg-gray-50 p-4 rounded-xl">
                  ...
                  <button
                    onClick={handleLogout}
                    className="w-full flex justify-center items-center gap-2 bg-primary text-white py-3 rounded-xl font-bold"
                  >
                    <LogOut size={18} /> Logout
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  <Link
                    onClick={() => setIsMobileOpen(false)}
                    to="/login"
                    className="text-center bg-blue-500 text-white py-3 rounded-xl font-bold"
                  >
                    Login
                  </Link>
                  <Link
                    onClick={() => setIsMobileOpen(false)}
                    to="/signup"
                    className="text-center bg-gray-800 text-white py-3 rounded-xl font-bold"
                  >
                    Signup
                  </Link>
                </div>
